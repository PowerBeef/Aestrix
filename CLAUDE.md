...
- `mlx-swift` is pinned to the **PowerBeef/mlx-swift fork** (branch `imarello/core-0.32.1`, revision-pinned; core v0.32.1, **nojit** Metal kernels — the full metallib is mandatory at runtime, a stub is fatal not slow; the companion mlx-c fork carries a compile-cache compat patch). Re-sync to upstream when ml-explore ships a 0.32.x mlx-swift release; any pin move = full bump-day validation (V-bench both canvases + 15/15 V-eval + vision).
