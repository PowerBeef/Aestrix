...
- **Done**: P0–P6c (macOS library + CLI: T2I, I2I, identity I2I, eval workflow), P8 (polish, CI floors), P9 (perf harness; Small Decoder + f16 scaled qmm are product defaults; `--text-tokens auto` was reverted to opt-in 2026-08-16 after a vision regression), the **2026-08-16 engine pass** (`Docs/ENGINE_RESEARCH.md` Tiers 0–2: quality fixes, reference-faithful tokenizer, byte-identical optimizations, joint-f16 attention, untiled 768² decode — 512² 23.0 s / 1024² 71.0 s / watermark 3.00 GiB), the **2026-08-18 audit + hardening pass** (`9ad69cd`: all 22 Critical/High/Medium findings fixed — durable print history, harness quarantine/sweep/param-threading, pipeline cancellation, atomic embed cache, CLI validation, 3-bit retirement, metallib gating, CI widened to 18 suites; remaining Low items listed in the audit artifact), and the **2026-08-18 engine-uplift session** (`8b9f51a`+`1a2cbfd`: mlx core 0.32.1 via fork — encode_te −19–21%, 512² 21.9 s; TE-splice as `--pad-content clean` (conditioning-only; TE is dispatch-bound — speed claim refuted); refine-regime scheduler; `t2i --two-stage` opt-in anatomy rescue; ChipCapabilities probe; iOS floor 26.2). The **2026-08-18 uplift continuation** (`1b5c397`) closed the batch: S4 (full-f16 single-stream epilogue + dynamic amax scale) **measured and closed** (+1.9–2.8% e2e, only win −150 MiB watermark @512², quality clean — `--attn-f16-full-epilogue` / `--attn-dynamic-scale` stay opt-in experiments); Lanczos two-stage upscale **refuted** (band-limited like bicubic — the re-gate needs an SR stage); fork thread-local-stream fix (86/86 tests); upstream filings mlx#4350 + mlx-swift#457. **That state is tagged `checkpoint-2026-08-18-engine-uplift`** — the validated restore point; work after it is experimental research until it re-passes the gates (ROADMAP Checkpoint section). **2026-08-18 evening (user decision): the bare-metal track went mainline** — `research/bare-metal` merged into `main` after its gates (86/86 tests, eval-regression 15/15, per-milestone oracle verification). The **direct-dispatch engine** (`Sources/ImarelloDirect`, study `Docs/BARE_METAL_RESEARCH.md`) drives MLX's metallib kernels from hand-built command buffers: TE encode shipped as `t2i --te-engine direct` — **the T2I default since 2026-08-18 late session** (FULL gate: 22/22 pixel incl. regression×3 seeds + anatomy probes + 1024² subjects, vision-clean; `--te-engine mlx` = escape hatch; i2i unchanged) (full window pixel 98.5 + vision PASS; with `--pad-content clean` the TE stage drops 1.6 s → **204 ms**). Hard-won facts: the TE must run **bf16** (real activations overflow f16); steel single-head masks need broadcast strides {0,0,L}; `AttnParams` is 56 scalar bytes + strides, no padding. **The prompt-embed cache was silently dead after `9ad69cd`** (`.float32` validation vs bf16 entries — every load deleted + re-encoded); fixed and verified. The **direct-VAE port closed (2026-08-19)**: V2–V5 ladder oracle-exact (cosine 1.0000000), then steel-GEMM attention + a 3-pass coalesced GroupNorm rewrite (the 138.7→9.1 ms whale) made the direct decoder **faster than the product** (untiled 512² 808 vs 932 ms; tiled 1024² 4.16 vs ~4.5 s) — `direct-generate` now decodes on `DirectVAE` (validated: fox 512² + bikini 1024² pixel+vision PASS); MLX's compute role there is entry math only. **`t2i --vae-engine direct` is the product default (2026-08-19)** — gate: 15/15 regression, **≤1 LSB byte-equivalence vs the MLX decode at both canvases** (0.0036%/0.0034% of subpixels, max delta 1 — the anatomy re-sweep waived on that basis), fox+bikini 1024² vision PASS, bench decode −11.6/−11.7% (931→823 ms @512², 4.62→4.08 s @1024²), e2e −0.8/−1.5% (21.63 s / 66.5 s), watermark flat, RSS +48–78 MB (1.9 GB peak, safe); `--vae-engine mlx` = escape hatch; i2i decode unchanged. Injection seam: `ImarelloPipeline.setPackedDecoder` (protocol `PackedLatentDecoding`; engine builds lazily at decode, staged residency preserved; bench `--vae-engine` + provenance schema 1.5). A post-promotion conv-tile fix (bn=32 for N=96 — MLX's own bn=64 heuristic pads 25%) took decode to **772 ms @512² / 3.94 s tiled @1024²**, byte-identical outputs; **Winograd is parked with numbers** (~100–150 ms @512² left for 3 new ABIs). **`t2i --dit-engine direct` is the product default (2026-08-19)** — the bespoke DiT behind `ImarelloPipeline.setPackedDenoiser` (`PackedLatentDenoising`; per-step cancellation + trace preserved; CLI-level injection, so the iOS app keeps the MLX DiT). FULL gate: 15/15 regression, 12/12 anatomy pixel with 11/12 vision-clean (dancer-s42 @512² three-arm = **model-level**, MLX control identical; dancer-s42 @1024² clean on both), 1024² trio vision PASS, same-seed drift mean 1.26/255. Bench: **e2e −6.7% @512² (20.6 s) / −9.0% @1024² (60.3 s)**, denoise/step −12%, MLX watermark −20% (1.77 GB), peak RSS +4–6% (~1.99 GB, safe). Constraints: requires `--text-tokens 512`, rejects `--pad-keep`/`--pad-bias`; attention-tuning flags are MLX-path no-ops; `--dit-engine mlx` = escape hatch; **i2i/identity stay on the MLX DiT** (not ported). **N-track (2026-08-19): the direct engine owns NAX; A19 Pro gate PASSED** — `affine_qmm_t_nax` ABI proven on the iPhone 17 Pro (g18p, iOS 26.6): cosine 1.0000000, 99.5% bit-exact, **3.38× vs Steel** at the DiT shape (40.0→11.8 ms); `DirectDiTStep(useNAXQmm:)` + `direct-generate --nax` (arch-probe-gated — PSO creation succeeds even on M2, never gate on it); `ImarelloSpikes` device runner ships a full `-sdk iphoneos` metallib resource. **WARNING: the main iOS app is latently broken on device since nojit** — its Xcode-built Cmlx metallib is the thin JIT-era 4.5 MB set; ship the full metallib + point MLX at it before any device generate. Next engine steps: device dit-step + full on-device bespoke generate (weights for the runner), iOS app metallib fix, i2i/identity DiT port decision. **Next engine levers**: N2/N3 A19 Pro NAX build + device A/B (iPhone must be on iOS ≥ 26.2); two-stage texture re-gate needs an **SR stage** (Lanczos measured 2026-08-18: within noise of bicubic — interpolation is not the bottleneck; `--refine-upscale lanczos` kept as an option).
