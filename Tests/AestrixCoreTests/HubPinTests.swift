...
struct HubPinTests {
    ...
    func defaultConfigUsesFourBitPin() {
        let config = AestrixConfig(tier: .low, maxSide: 512)
    ...
    func applyPresetKeepsPin() {
        var config = AestrixConfig(weightPreset: .bits4, tier: .low, maxSide: 512)
    ...
    func parsesHuggingFaceMetadata() throws {
        let tmp = FileManager.default.temporaryDirectory
            .appendingPathComponent("aestrix_hubpin_\(UUID().uuidString)", isDirectory: true)
