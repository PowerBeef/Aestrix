...
struct IdentityPreserveTests {
    ...
    private func writeSolidPNG(width: Int, height: Int, r: UInt8, g: UInt8, b: UInt8) throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("imarello-face-test-\(UUID().uuidString).png")
        ...
        guard let ctx = CGContext(
            ...
            throw ImarelloError.imageLoadFailed(path: url.path, reason: "test PNG context failed")
        }
        guard let dest = CGImageDestinationCreateWithURL(url as CFURL, "public.png" as CFString, 1, nil)
        else {
            throw ImarelloError.imageLoadFailed(path: url.path, reason: "destination failed")
        }
        CGImageDestinationAddImage(dest, cg, nil)
        guard CGImageDestinationFinalize(dest) else {
            throw ImarelloError.imageLoadFailed(path: url.path, reason: "finalize failed")
        }
        return url
    }
}
