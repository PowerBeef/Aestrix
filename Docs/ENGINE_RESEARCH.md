...
**2026-08-18 addendum (engine-uplift session):** §5.3 core bump SHIPPED via maintained fork (nojit; see PERF.md 2026-08-18 — encode_te −19–21%, watermarks flat; upstream JIT affine_qmv template-arity bug worth reporting to ml-explore). §5.1 TE-splice speed estimate **stands corrected**: implemented and measured — the TE encode is dispatch-bound on M2, so real-token-only encoding is speed-neutral; the splice ships for its conditioning semantics only. §4.6(b) do_causal is subsumed by the splice's padless encode. Two-stage 1024² compose→refine fixes few-step body-plan failures (the open P7 anatomy item has a working opt-in rescue) but needs a real SR stage before any default gate. NAX runtime detection recipe (from core's `is_nax_available()`): Metal architecture gen ≥ 17 ('g')/18 ('p') + OS 26.2 — supersedes name heuristics for the N-track.

**2026-08-18 addendum 2 (S4 + stream fix + upstream reports):** §5.2 full-f16 single-stream epilogue **measured and refuted for speed** (PERF.md S4 section): the qmm already computes in f16, so the f16 epilogue costs +1.9–2.8% e2e; the only real effect is −150 MiB watermark @512² (non-streamed path); 1024²'s watermark lives in full-length attention tensors the epilogue never touches. Quality fully clean (pixel + vision, both canvases, dynamic amax scale included) → `--attn-f16-full-epilogue` / `--attn-dynamic-scale` stay opt-in experiment flags; the "better activation scale" open item is answered (works, costs ~1% extra, buys nothing at product sizes). **Core ≥0.31.2 thread-local streams**: mlx made `default_stream()`/CommandEncoders per-thread (mlx#3348), which breaks mlx-swift's process-global `Stream.gpu`/`Stream.cpu` statics — cross-thread eval aborts ("There is no Stream(gpu, 0) in current thread"; surfaced by parallel Swift Testing suites, latent for any thread-hopping pipeline). Fixed in the fork at the mlx-c layer: `mlx_default_{cpu,gpu}_stream_new` return a process-wide `new_thread_unsafe_stream` (0.31.x semantics); tests additionally clone the full metallib into the xctest bundle (nojit requirement; `Tests/ImarelloCoreTests/TestMetallib.swift`). Upstream reports filed: ml-explore/mlx#4350 (0.32.1 JIT affine_qmv template arity), ml-explore/mlx-swift#457 (thread-local stream break), field-confirmation + working-patch offers on mlx-swift#446 and PR #450.
