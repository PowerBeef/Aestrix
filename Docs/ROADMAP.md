...
**Last updated:** 2026-08-19 — **the product T2I path is fully bespoke.** The direct-dispatch engine now owns every stage by default (`--te-engine direct` since 2026-08-18; `--vae-engine direct` and `--dit-engine direct` promoted 2026-08-19 through FULL gates): 512² **20.6 s** / 1024² **60.3 s** on the 8 GB M2 (from 23.0 / 71.0 at the start of the engine ladder), MLX watermark −20%, identical-or-byte-equivalent quality (VAE: ≤1 LSB; DiT: same-seed drift class, 15/15 + 12-probe anatomy + 1024² trio gated). The VAE port (V2–V5, oracle cosine 1.0000000) beat the MLX decode after a steel-GEMM attention swap, a 15× three-pass GroupNorm rewrite, and an alignment-aware conv-tile fix. **NAX landed in the engine and PASSED its A19 Pro device gate: 3.38× vs Steel on the DiT qmm** (`ImarelloSpikes` runner, iPhone 17 Pro g18p, cosine 1.0). i2i/identity stay on the MLX DiT; `--*-engine mlx` are escape hatches. **BLOCKING: the main iOS app is latently broken on device since the nojit switch** (thin Xcode-built Cmlx metallib) — see P7. Earlier history (2026-08-18, three passes): the **audit + hardening pass** (`9ad69cd`), the **engine-uplift session** (`8b9f51a`…`2f4fbb8`: mlx core 0.32.1 via fork, TE-splice as `--pad-content clean`, `t2i --two-stage`, ChipCapabilities, iOS floor 26.2), and the **uplift continuation** (`1b5c397`: S4 measured and closed, fork thread-local-stream fix, Lanczos refuted, upstream filings). **This state is tagged `checkpoint-2026-08-18-engine-uplift`** — see the Checkpoint section below.  
...
| Field | Detail |
|-------|--------|
| **Status** | `partial` — **and REGRESSED on device since 2026-08-18 (nojit)**: the app's Xcode-built Cmlx-bundle metallib is the thin JIT-era set (4.5 MB vs the required 155 MB), so any MLX op on device aborts on kernel lookup. Fix before the next device generate: embed a full `-sdk iphoneos` metallib (recipe proven in the `ImarelloSpikes` target) and point MLX at it. Pre-nojit state: 512² T2I + I2I pass pixel + vision on device (2026-08-16). Simulator = UI only. Open items: the metallib fix, then 1024² vision anatomy |
...
- [ ] **Full nojit metallib in the app bundle + MLX pointed at it** (blocking; broken since 2026-08-18 nojit — proven recipe in `ImarelloSpikes`)  
