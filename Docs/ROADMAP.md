...
**Last updated:** 2026-08-21 — **Metal Engine V2 foundations are implemented but not promoted.** The current product T2I path remains the qualified V1 Direct route: 512² **20.57 s** / 1024² **60.34 s** on the 8 GB M2, treated as historical references until reproduced on this worktree. V2 now has a whole-T2I actor-isolated backend seam, internal deterministic `ImarelloPlan`, strict artifact/ABI and safetensors foundations, a build-time Direct metallib, legacy-plan replay, explicit memory accounting, batched conditioning, opt-in captures, an isolated Metal 4 executor, and fail-closed iOS integration. The V2 shell cannot become the product route until tensor/trajectory/image parity, A/B performance and memory, reliability, and physical-device gates pass. i2i/identity stay on MLX. **BLOCKING: the full main-app `iphoneos` artifact integration is coded but has not been physically verified on a signed device**, so device generation remains unclaimed; see P7. Earlier measured history remains below.
**Working tree focus:** qualify V2 slice by slice while preserving V1 and closing the P7 signed-device artifact gate.
**DIRECTION (2026-08-18 evening, user decision): the custom direct-dispatch engine is mainline.** `research/bare-metal` merged into `main`; Imarello now pursues its own Metal engine stage by stage (TE landed as opt-in `--te-engine direct`; DiT/VAE follow through the same gates). MLX remains the kernel source (metallib) and the DiT/VAE runtime today. The `checkpoint-2026-08-18-engine-uplift` tag remains the pre-merge restore point. Study: `Docs/BARE_METAL_RESEARCH.md`.
**Engine-uplift track is ACTIVE.** Landed 2026-08-19: direct VAE (V2–V5 + speed pass, product default), direct DiT (product default, e2e −6.7/−9.0%), **NAX qmm proven on the A19 Pro (3.38×, `DirectNAX` + `ImarelloSpikes`)**. Next levers, in order: ① **iOS app full-metallib fix** (blocking — device generates broken since nojit), ② device dit-step spike + first on-device bespoke generate (weights for the spike runner), ③ iOS direct-DiT+NAX product decision (projected ~1.4–1.5× device denoise), ④ i2i/identity DiT port decision, ⑤ two-stage **SR texture re-gate** (needs an SR-model product decision). Parked without a further ask: Winograd conv (~100–150 ms @512² for 3 ABIs), TAEF2 preview, ref-KV, Δ-DiT, `stagedAggressive`.  
...
**Audit remediation and iOS redesign working tree (2026-08-19):** full-pack metallib rejection, odd-token direct-DiT coverage, recoverable direct-VAE allocation/tile failures, process-wide Metal serialization, typed request/tensor/snapshot validation, prompt-cache v4 identity, benchmark schema 1.7/comparability, image-analysis schema 1.5/tile provenance, transactional harness/image persistence, privacy manifest, and iOS unit/UI test targets are implemented. The iOS app now uses persistent native Create/Edit/Gallery/Settings tabs, independent navigation stacks and drafts, typed global generation activity with immutable retry provenance, seed-only Generation Options, scene-aware harness monitoring, asynchronous image loading, and pushed image detail while preserving the 1024² default and frozen persistence/harness contracts. Runtime quality, benchmark A/B, memory, and physical-device evidence remain intentionally unclaimed; the main target now has full-artifact build wiring, but the signed installed bundle and device execution are unverified.

**Metal Engine V2 working-tree state (2026-08-21):** implemented foundations include whole-pipeline preflight selection with pending backend swaps; a Metal-free plan target and independent placement verifier; strict Direct/MLX artifact descriptors and ABI registry; source-built `imarello-direct.metallib` plus deterministic manifest; legacy raw scratch reproduction at 180/318.75/513 MiB; explicit memory ledger; multi-step conditioner packing; strict mapped safetensors and V2-only atomic prompt cache; optional capture manifests; exact Steel/NAX attention inventories; an OS-gated Metal 4 argument-table/constant-arena/residency/feedback executor; and main-app build wiring for both metallibs. Focused MLX-free tests and a UI-only Simulator build pass. Still open: complete V1 captures/tolerances, block/tile capture points, native mapped loading in every stage, Tier B/C streaming, Metal 4 qmm/attention qualification, full TE/VAE handoff removal, A/B performance and image gates, and signed physical-device qualification. No new speed, memory, quality, or device-runtime claim follows from these foundations.
...
| ID | Item | Where / notes |
|----|------|----------------|
| P0 | Scaffold, SPM packages, staged memory policy, BFL skills | `Package.swift`, `Docs/MEMORY.md`, `.agents/skills/flux-best-practices/` |
...
| Field | Detail |
|-------|--------|
| **Status** | `partial` — the main target now has a build phase that validates and embeds the full ~155 MB MLX pack plus `imarello-direct.metallib` and its manifest, and `GenerationEngine` resolves both before pipeline construction. This has compiled on the UI-only Simulator but has **not** been signed, installed, or exercised on a physical device. Keep the no-JIT device blocker open until the bundled hashes/symbols and first serialized 512² run pass on-device. Pre-no-JIT 2026-08-16 evidence remains historical. |
...
- [x] Metallib check rejects stubs/thin JIT-era packs before pipeline construction
...
- [x] Staged config with repository-locked 1024 default and explicit 512 selection
...
- [x] Main-target build integration validates/copies the full no-JIT MLX pack and Direct artifact set; `GenerationEngine` resolves them before pipeline construction
- [ ] **Physical signed-app verification that both artifacts are bundled, selected, and executable** (blocking)
...
Do not drive a device generate matrix yet. Build and sign the main `iphoneos` target with both generated artifact sets, inspect the installed bundle/readiness gate, resync weights, and run one serialized 512² bring-up case before the locked 1024² product case. IDs are bounded safe basenames and existing running/done IDs are rejected. Do not generate on the Simulator. Recipe: [`IOS.md`](IOS.md).
...
**Latest default-path (8 GB Mac mini, release, fully bespoke T2I — direct TE+conditioning+DiT+VAE, 2026-08-19):** 512² W1T3 e2e **20.6 s** / denoise **3.77 s**/step / decode **0.79 s**; **1024² e2e 60.3 s** / denoise **12.93 s**/step / decode **3.91 s**; identity-i2i 512² **36.2 s** (MLX DiT path, unchanged); MLX watermark **1.77 GiB** (−20% — conditioning + DiT left MLX), peak RSS ~2.0 GB. *(The 2026-08-18 MLX-engine row — 21.9 / 67.5 s, watermark 2.57 / 3.00 GiB — is the pre-engine baseline.)* Full tables: `Docs/PERF.md`; engine story: `Docs/BARE_METAL_RESEARCH.md`.
...
2. Default next item is the **P7 iOS full-metallib fix** (device generates are broken without it), then the on-device direct-engine ladder (dit-step spike → bespoke generate → NAX product decision). P9 leftover speed work stays **paused**.  
...
| 2026-08-16 | **Historical Spread Deck UI.** Two full-bleed Stage/Contact Sheet pages replaced the original form and introduced edit-from-any-print. Its later-removed Dynamic Type cap and page navigation are recorded only as history; the 2026-08-19 native-tab redesign supersedes its UI authority. Device redeploy + resync then produced historical pre-no-JIT 512² T2I/I2I pixel + vision passes. |
| 2026-08-19 | **iOS app reengineered around native iOS 26 structure.** Persistent Create/Edit/Gallery/Settings tabs own independent navigation stacks; Create and Edit retain independent drafts; Edit is source-first and iterative; image detail is pushed with system back; `tabViewBottomAccessory` routes typed activity to its originating workspace; Generation Options owns seed only; image decode/storage accounting are actor-isolated; Photos feedback is detail-local; and harness polling is scene-aware. The persistence index and frozen harness schema/paths are unchanged. Simulator build plus 25 unit and 7 UI tests are green; device generation remains blocked on the full no-JIT metallib. |
...
| 2026-08-18 | **Bare-metal Stage 2 landed and merged to main (user decision: custom implementation is mainline).** Direct-dispatch engine (`ImarelloDirect`): MLX's metallib kernels driven from hand-built command buffers over a static buffer plan. Milestones, all oracle-verified: **A** qmm 100% bit-exact; **B** full Qwen3 layer, one command buffer, cosine 1.0 (AttnParams = 56 scalar bytes, no padding; single-head masks need broadcast strides {0,0,L}); **C** 27 layers in one CB — 1.48× vs the resident encode, L=30 in 79 ms; **D** production-shaped encode in **bf16** (f16 overflows on real Qwen activations — taps reach |1.5e4|), real-row fidelity 0.9999999, fox pixel 98.5 + vision PASS via `t2i --te-engine direct`; **E** splice — TE stage **1.6 s → 204 ms (7.8×)**, drift LPIPS 0.222 in the R4 band, vision clean. **Product bug found en route and fixed: the prompt-embed cache never hit after `9ad69cd`** (audit validation demanded `.float32` vs the TE's bf16 entries → silent delete+re-encode every generate); fix verified (repeat generates leave entries untouched, ~2 s faster). `--te-engine direct` stays **opt-in** until the FULL promotion gate. Merge gate: 86/86 filtered tests + eval-regression 15/15. |
| 2026-08-18 | **Platform floors raised to macOS 26.2 / iOS 26.2** (user decision, on `research/bare-metal`). Rationale: every Apple Silicon Mac runs macOS 26, so no supported chip is dropped — the floor only requires an OS update; the host of record is on 26.6.1. Buys: Metal 4 (MTLTensor, MPP tensor ops) as a *single* API target for the bespoke-engine ladder, and MLX's NAX host dispatch compiled in everywhere (fork `079609a` drops `MLX_METAL_NO_NAX`; `is_nax_available()` runtime-gates execution to A19/M5-class GPUs, so M2 takes byte-identical kernels). Buys nothing on M2 perf. Supersedes the earlier "macOS floor stays 15" constraint in the engine-uplift plan. Validation on the M2: build + metallib + filtered tests + V-bench-neutral check before any merge to main. |
| 2026-08-18 | **Engine-uplift continuation (S4 + fork stream fix + upstream reports).** **S4** full-f16 single-stream epilogue measured and **refuted for speed** (+1.9–2.8% e2e; only win = −150 MiB watermark @512²; 1024² watermark untouched — it lives in full-length attention tensors; quality clean at both canvases incl. the dynamic amax scale) → `--attn-f16-full-epilogue` / `--attn-dynamic-scale` stay opt-in experiments, S4 closed. **Fork stream fix**: mlx core ≥0.31.2 thread-local CommandEncoders break mlx-swift's process-global `Stream.gpu`/`.cpu` statics (cross-thread eval aborts; surfaced by parallel test suites, latent for any thread-hop) — fixed at the mlx-c layer (`mlx_default_*_stream_new` → process-wide `new_thread_unsafe_stream`; fork pins moved to mlx-swift `b0605dc` / mlx-c `3df95f5`), plus `TestMetallib` clones the full metallib into the xctest bundle (nojit requirement). **Upstream**: filed ml-explore/mlx#4350 (0.32.1 JIT affine_qmv arity) and mlx-swift#457 (stream break); working patches offered on mlx-swift#446/PR#450. `t2i --two-stage --refine-upscale lanczos` added (CoreImage CPU Lanczos) and **measured same-day: refuted as the texture fix** (fox 69.3 vs bicubic 72.6; dancer 64.4 vs 60.8; anatomy rescue holds on vision) — band-limited upscales all starve the low-σ refine; the re-gate needs a real SR stage (new dependency, needs a product decision). |
| 2026-08-18/19 | **Direct-VAE port (V2–V5) → product default.** Ladder oracle-exact at every rung (resnet, mid+attention, full Small Decoder @512², tiled 1024² via the product stitcher — cosine 1.0000000). Speed chase, measurement-led: steel-GEMM attention (new fused-GEMM ABI; mid block 390→100 ms) then the real whale — the spike GroupNorm (138.7 ms @512²×192, 32 uncoalesced TGs) rewritten as a 3-pass coalesced reduction → **9.1 ms (15×)**; decode 1.96 s → **808 ms untiled 512²** / 10.3 → **4.16 s tiled 1024²** — faster than the MLX decode. Promotion gate: same-seed outputs **≤ 1 LSB** from the MLX engine at both canvases (0.0036% of subpixels; anatomy re-sweep waived on that basis), 15/15 regression, vision PASS, bench decode −11.6/−11.7%, watermark flat. `--vae-engine mlx` escape hatch; i2i decode unchanged. Post-promotion conv-tile fix (bn=32 for N=96 — MLX's own bm64→bn64 heuristic pads 25%): decode **772 ms / 3.94 s**, byte-identical; **Winograd parked with numbers**. Pipeline seam: `PackedLatentDecoding` + lazy engine build (staged residency preserved). |
| 2026-08-19 | **Direct-DiT → product T2I default; the product path is fully bespoke.** Second pipeline seam (`PackedLatentDenoising`; per-step cancellation + trace preserved; engine loads/frees its 1.93 GiB inside the call). Pixel-changing flip (final-latent cosine ≈0.9999, image drift mean 1.26/255) → FULL gate on its own merits: 15/15 regression; **full 12-probe anatomy set** pixel-clean, 11/12 vision-clean — the twelfth (dancer-s42 @512², three arms) reproduced **composition-identically on the MLX control** = model-level, both engines faithful; 1024² trio vision PASS. Bench: e2e **22.04→20.57 s @512² (−6.7%)**, **66.28→60.34 s @1024² (−9.0%)**, denoise/step −12%, **MLX watermark −20% (1.77 GiB)**, RSS ~1.99 GB (safe). Constraints: 512 text tokens, no pad-keep/pad-bias; attention-tuning flags are MLX-path no-ops; **i2i/identity stay MLX**; `--dit-engine mlx` escape hatch. CLI-level injection ⇒ **iOS keeps the MLX DiT** (preserves MLX's free NAX dispatch pending the engine owning it). Bench `--vae-engine`/`--dit-engine` + provenance schema 1.5/1.6; `eval-anatomy.sh` honors `T2I_EXTRA`. |
| 2026-08-19 | **NAX in the direct engine — A19 Pro device gate PASS (user decision: engine owns NAX, not the MLX pathway).** `DirectNAX` mirrors core's recipe (OS ≥ 26.2, arch gen ≥ 17 / 'p' parts ≥ 18) and the `qmm_nax` ABI (identical buffer layout to steel qmm; 64-tile grid; K%64, non-f32). `DirectDiTStep(useNAXQmm:)` covers every DiT projection; `direct-generate --nax`; refused on ineligible GPUs. Device gate on the iPhone 17 Pro (**g18p gen 18 — the A19 question answered YES**, iOS 26.6, `ImarelloSpikes` runner): **cosine 1.0000000, 99.5% bit-exact, 40.0→11.8 ms = 3.38×** at M=4096/K=3072/N=3072 — projected ~1.4–1.5× device denoise. No NAX attention kernels exist in 0.32.1 — qmm is the whole prize. Hard-won: **PSO creation for NAX kernels succeeds on non-NAX GPUs** (arch probe is the only gate); the spike generates tensors **MLX-free** on device. |
| 2026-08-19 | **Two latent 0.32.1-fork breakages found via the first post-bump device build, fixed at the fork (`e5b42df`):** ① core's CPU `jit_compiler.cpp` calls `std::system` (absent on iOS) → excluded on Apple platforms + a four-symbol stub (`available()`=false ⇒ interpreted CPU fallback; GPU untouched; macOS byte-identical + 86/86 after the pin move). ② **The app-embedded Cmlx metallib is the thin JIT-era set (4.5 MB vs 155 MB)** — Xcode auto-compiles the .metal sources without nojit instantiations ⇒ **the main iOS app is broken on device since the nojit switch** (any MLX op aborts on kernel lookup; last device validation predates nojit). The `ImarelloSpikes` target proves the fix: full `-sdk iphoneos` metallib as an app resource, engine takes it by URL. Main-app fix = same embed + point MLX at it (P7 blocking item). |
