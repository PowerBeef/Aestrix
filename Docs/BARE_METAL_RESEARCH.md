...
Stage 2's ladder is complete: both gates cleared, the full pipeline runs on bespoke TE + DiT with MLX reduced to conditioning math and the VAE, and every stage is oracle-verified. What remains for productization: persistent-process reuse of the engines (kill the per-run build/load), the conditioning math on-engine (drops the product-module load from the flow), direct VAE (the last MLX stage), and the promotion path for `--te-engine`/`direct-generate` defaults through the FULL gate.
