...
6. **VAE**: T2I / final I2I decode use **decode-only** weights (~97 MB). Large canvases use **tiled decode** (`VAETileConfig`: default overlap + cosine blend). Optional BFL **Small Decoder** via `--vae-variant small-decoder` (decode only). **I2I encode is always the klein AE** — do not load `full_encoder_small_decoder.safetensors`. **Default decode stays full AE.**
...
| `imarello load-te` / `load-dit` / `load-vae` | Staged weight load smoke; `load-dit --dump-dtypes` = compute-dtype probe; `load-vae --vae-variant small-decoder` |
