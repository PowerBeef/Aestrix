...
6. **VAE**: T2I / final I2I decode use **decode-only** weights (~97 MB). Large canvases use **tiled decode** (`VAETileConfig`: default overlap + cosine blend). Optional BFL **Small Decoder** via `--vae-variant small-decoder` (new module, not a klein-pack swap). **Default stays full AE.**
