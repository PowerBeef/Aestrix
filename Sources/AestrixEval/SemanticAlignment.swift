...
///
/// - **Core ML CLIP** when models exist under the Aestrix CLIP cache directory
...
public enum SemanticAlignment {
    ...
    /// Default Core ML model directory.
    public static var defaultModelDirectory: URL {
        FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("Library/Caches/Aestrix/models/clip-coreml", isDirectory: true)
    }
    ...
    private static func coreMLCLIPScore(
        ...
        if textModel.modelDescription.inputDescriptionsByName[textInputName]?.type == .string {
            ...
            throw AestrixError.imageLoadFailed(
                path: txtURL.path,
    ...
    private static func firstMultiArray(from provider: MLFeatureProvider) throws -> MLMultiArray {
        ...
        throw AestrixError.imageLoadFailed(path: "clip", reason: "no multiarray embedding in model output")
    }
    ...
    private static func makeCVPixelBuffer(cgImage: CGImage, side: Int) throws -> CVPixelBuffer {
        ...
        guard status == kCVReturnSuccess, let buffer = pb else {
            throw AestrixError.imageLoadFailed(path: "clip", reason: "CVPixelBufferCreate failed")
        }
        ...
        guard let ctx = CGContext(
            ...
            throw AestrixError.imageLoadFailed(path: "clip", reason: "CGContext for CLIP resize failed")
        }
