...
enum ImageImport {
    ...
    static func loadCGImage(url: URL, width: Int, height: Int) throws -> CGImage {
        guard FileManager.default.fileExists(atPath: url.path) else {
            throw ImarelloError.imageLoadFailed(path: url.path, reason: "file not found")
        }
        guard let src = CGImageSourceCreateWithURL(url as CFURL, nil),
              let cgImage = CGImageSourceCreateImageAtIndex(src, 0, nil)
        else {
            throw ImarelloError.imageLoadFailed(path: url.path, reason: "not a decodable image")
        }
    ...
    private static func resizeAndCrop(_ image: CGImage, width: Int, height: Int) throws -> CGImage {
        ...
        guard let ctx = CGContext(
            ...
            throw ImarelloError.imageLoadFailed(path: "", reason: "resize context failed")
        }
        ctx.interpolationQuality = .high
        ctx.draw(image, in: CGRect(x: ox, y: oy, width: rw, height: rh))
        guard let out = ctx.makeImage() else {
            throw ImarelloError.imageLoadFailed(path: "", reason: "resize makeImage failed")
        }
        return out
    }
}
